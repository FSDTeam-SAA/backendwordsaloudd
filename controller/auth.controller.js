...
//   user.area = area?.trim() || "";
//   user.role =
//     role === "tradesman" ? "tradesman" : "client";
...
export const register = catchAsync(async (req, res) => {
  ...
  // Same OTP will be verified again
  if (!user.isOTPValid(otp)) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      "Invalid or expired OTP"
  ...
  // OTP will be removed only after successful registration
  // user.clearOTP();
...
export const sendSignupOtp = catchAsync(async (req, res) => {
  ...
  // Reset verification status if requesting a new OTP
...
export const login = catchAsync(async (req, res) => {
  ...
  // Login OTP use হয়ে গেলে clear করে দাও
  user.clearOTP();
...
export const verifyResetPasswordOTP = catchAsync(async (req, res) => {
  ...
  const user = await User.findOne({
    ...
    "+resetPasswordOtp.code +resetPasswordOtp.expiresAt"
  );
